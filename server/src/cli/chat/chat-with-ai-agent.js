...
const initConversation = async (userId, conversationId = null) => {
...
const agentLoop = async (conversation) => {
  ...
  while (true) {
    ...
    try {
      ...
      await saveMessage(conversation.id, 'assistant', `Error: ${err.message}`)
...
export const startAgentChat = async (conversationId = null) => {
  try {
    ...
    const errorBox = boxen(chalk.red(`❌ Error: ${err.message}`), {
      padding: 1,
      margin: 1,
      borderStyle: 'round',
      borderColor: 'red',
